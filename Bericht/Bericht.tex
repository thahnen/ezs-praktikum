...
\usepackage[ansinew]{inputenc}		% Nötig für deutsche Umlaute
\usepackage{scrpage2}				% Nötig für Fusszeile
\usepackage{graphicx}				% Nötig für eingebettete Bilder
\usepackage{listings}				% Nötig für Quelltext-Einbindung
\usepackage{courier}				% Nötig für Courier-Font (Quelltext)
\usepackage[bottom=10em]{geometry}	% Nötig für Grössenveränderung Fusszeile

\clearscrheadfoot
\pagestyle{plain}

% Geforderte "Formatvorlage" für Autoren; Parameter:
% - 1) Vor- & Zuname
% - 2) Matrikelnummer
\newcommand{\autor}[2]{#1, \\ & & Matrikelnummer #2}

% Geforderte "Formatvorlage" für Quellcode; Parameter:
% - 1) Sprache des Quellcodes (siehe: https://en.wikibooks.org/wiki/LaTeX/Source_Code_Listings#Supported_languages)
% - 2) Zeilengrösse (nicht Schriftgrösse!), Standard: 10
%
...
\textbf{Diagramm: Daten einlesen + \"ubermitteln Treiber}

\subsection{Strukturgramme der einzelnen Rechenprozesse}

\textbf{Diagramm: Strecke vermessen} \\
\textbf{Diagramm: Gegnerthread} \\
\textbf{Diagramm: Eigener Thread} \\
